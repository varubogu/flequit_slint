...
pub(super) async fn delete(
    ...
    let sqlite_result: Result<(), RepositoryError> = async {
        ...
        for tag_id in tag_ids {
            sqlite_guard
                .tag_bookmarks()
                .remove_all_by_tag_id_with_txn(&transaction, project_id, &tag_id)
                .await?;
            sqlite_guard
                .task_tags
                .remove_all_by_tag_id_with_txn(&transaction, project_id, &tag_id)
            ...
            sqlite_guard
                .subtask_tags
                .remove_all_by_tag_id_with_txn(&transaction, &tag_id)
                .await?;
            sqlite_guard
                .tags()
                .delete_with_txn(&transaction, project_id, &tag_id)
                .await?;
        }

        sqlite_guard
            .task_lists()
            .remove_all_by_project_id_with_txn(&transaction, project_id)
            .await?;
        sqlite_guard
            .projects()
            .delete_with_txn(&transaction, project_id)
            .await?;
        Ok(())
    }
    .await;
    ...
    if let Some(automerge) = repositories.unified_manager.automerge_repositories() {
        ...
        if let Err(error) = automerge_result {
