...
| 前タスク ID | previous_task_id | Option\<TaskId\> | - | NULL | tasks.id | 繰り返しで自動生成されたタスクの生成元。次タスク ID は持たない（補足参照） |
...
`project_id`, `task_list_id`, `status`, `due_date`, `order_index`, `created_at`, `(project_id, previous_task_id)`
...
繰り返しルールを持つタスクは、完了するたびに次の期限を持つ別タスク（別のタスク ID）を生成する。
Git のコミットが親だけを持つのと同じく、タスクは生成元の **前タスク ID** だけを持つ。
次タスクは「前タスク ID が自分であるタスク」を探して求め、次タスク ID は二重管理になるため持たない。

- **完了したとき**
  - 次タスクがまだ無く、次回の期限があれば次タスクを生成する
  - 次タスクが既にあれば生成しない（二重生成の防止）
- **キャンセルしたとき**
  - 次タスクが生成時点から変更されていなければ削除する
  - 変更されていれば削除せず残す。残した状態で再び完了しても、次タスクが既にあるため生成しない
- **次タスクの内容**
  - 期限は繰り返しルールで求めた次回日時にする。起点は完了したタスクの期限で、期限が無ければ現在時刻
  - 開始日時とリマインダーは期限と同じだけずらす
  - タイトル・詳細説明・優先度・所属リスト・担当者・タグを引き継ぐ
  - ステータスは未着手に戻し、実績日時は引き継がない。サブタスクは引き継がない
  - 繰り返しルールは複製して次タスク専用にする。共有すると、次タスクで繰り返しを解除したときに前タスクのルールまで消えるため
- **系列の回数**：前タスク ID を辿った数で数える。`max_occurrences` に達したら生成しない。`end_date` を過ぎる場合も生成しない
- **「変更されていない」の判定**：以下をすべて満たすこと
  - タスクの `updated_at` が `created_at` と等しい
  - サブタスクが無い
  - タグが前タスクと同じ
  - 繰り返しルールが残っていて、ルールの `updated_at` が `created_at` と等しい

  タグ・サブタスク・繰り返しルールは別テーブルで管理しており、変更してもタスクの `updated_at` は動かない。そのため個別に確認する
